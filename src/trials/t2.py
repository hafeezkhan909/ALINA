...
def combine_features(color_features, texture_features, gradient_features, w1, w2, w3):

    combined_features = w1 * color_features + w2 * texture_features + w3 * gradient_features
    combination_features = combined_features.astype(np.uint8)
    cv2.imwrite('/home/hafeez/Desktop/combined_features.jpg',combination_features)
    cv2.imshow('MMMMMMM', combination_features)
...
def gradient_normalization(image):
    ...
    # cv2.imwrite('/home/hafeez/Desktop/gradient_magnitude.jpg', gradient_magnitude)
...
def extract_road(image, kmeans_model):
    ...
    multi_features = combine_features(color_features, texture_features, gradient_features, 0.3, 0.5, 0.2)
    multi_features = multi_features.reshape(-1, 3)
...
# Train KMeans model
kmeans = KMeans(n_clusters=8, init='k-means++', n_init=10, max_iter=5000, tol=0.001, verbose=0, random_state=None, copy_x=True)
...
keypoints, descriptors = sift.detectAndCompute(multi_features, None)

# Draw the detected keypoints on the image
img_with_keypoints = cv2.drawKeypoints(multi_features, keypoints, None)
